...
class Module {
	...
	public function setConfigModule($e) {
		...
		$baseurl      = $folderPublic . $conf['router']['routes']['wread']['options']['route'];
		...
		\Wread\Util\Route::set($subru);

		switch ($subru) {
		case 'ajax':
		case 'json':
			$viewModel->setTemplate('layout/wread/empty');
			break;
		default:
			$viewModel->setTemplate('layout/wread');
			break;
		}

		$viewModel->nAppBuild = "";
		if (ENVIRONMENT == 'development') {
			$request->setBasePath($folderPublic . '/f/wread-dev/');
		} else {
			$request->setBasePath($folderPublic . '/f/wread/');
